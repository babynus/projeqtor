...
class Menu extends SqlElement {
  ...
  public static function drawNewGuiMenu($menu, $defaultMenu, $customMenuArray,$isReportMenu=false,$isAnotherBar=false) {
    ...
    if($isReportMenu==true){
      ...
      echo 'onClick="editFavoriteRow(false);loadMenuReportDirect(\''.$menu->idReportCategory.'\',\''.$menu->id.'\');refreshSelectedMenuLeft(\''.$menu->name.'\');showMenuBottomParam(\'' . $class .  '\',\'true\')">';
      ...
      if ($menu->type=='item') {
      	...
      	echo 'onClick="editFavoriteRow(false);hideReportFavoriteTooltip(0);loadMenuBarItem(\'' . $class .  '\',\'' . htmlEncode(i18n($menuName),'quotes') . '\',\'bar\');refreshSelectedMenuLeft(\''.$menuName.'\');showMenuBottomParam(\'' . $class .  '\',\'false\');"';
      	echo 'oncontextmenu="event.preventDefault();hideReportFavoriteTooltip(0);';
        ...
        echo ' onMouseLeave="hideFavoriteTooltip(0,\''.$class.'\');"';
        echo 'onClick="editFavoriteRow(false);loadMenuBarPlugin(\'' . $class .  '\',\'' . htmlEncode(i18n($menuName),'quotes') . '\',\'bar\');refreshSelectedMenuLeft(\''.$menuName.'\');showMenuBottomParam(\'' . $class .  '\',\'false\');">';
        Menu::drawIconMenuNewGui($drawMode, $class, $menu);
        ...
        if (securityCheckDisplayMenu($idMenu, $class)) {
        	...
        	echo ' onMouseLeave="hideFavoriteTooltip(0,\''.$class.'\');"';
        	echo 'onClick="editFavoriteRow(false);loadMenuBarObject(\'' . $class .  '\',\'' . htmlEncode(i18n($menuName),'quotes') . '\',\'bar\');refreshSelectedMenuLeft(\''.$menuName.'\');showMenuBottomParam(\'' . $class .  '\',\'true\');">';
             Menu::drawIconMenuNewGui($drawMode, $class, $menu);
        	Menu::drawNewGuiDialogueAddRemoveFav($menu,$customMenuArray,$defaultMenu,$class);
        	echo '</div>';
        	echo '</div>';
