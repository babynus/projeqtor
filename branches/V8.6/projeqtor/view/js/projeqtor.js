     ...
     // last operations depending on the executed operatoin (insert, delete, ...)
        ...
        if (dojo.byId("buttonDivObjectId")
          ...
          if (lastOperation.value=='insert' && dojo.byId('directLinkUrlDivDetail')) {
            ...
            valueDiv+= '&nbsp;<a href="'+ref+objId+'" onClick="copyDirectLinkUrl(\'Button\');return false;" title="'+i18n("rightClickToCopy")+'" style="cursor: pointer; color: white;" onmouseover=this.style.color="black" onmouseout=this.style.color="white">';
...
function resizeListDiv() {
  ...
  dojo.query(".allSearchFixLength").forEach(function(node, index, nodelist) { 
      var nodeWidth=node.offsetWidth+5;
  ...
  for (var i=0;i<arrayFieldsOrder.length;i++) {
    ...
    if (arrayFields[fld]["visible"]==false) {
      ...
      dojo.query(widgetId).forEach(function(node, index, nodelist) { node.style.width=(fldWidth)+"px";});  
    }
