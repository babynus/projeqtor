...
$version = "V9.2.0"; // Version of application : Major / Minor / Release
$build = "0287.1"; // Build number. To be increased on each release
  ...
  if(!sessionValueExists('startAm') or !sessionValueExists('endAm') or !sessionValueExists('startPm') or !sessionValueExists('endPm')){
    ...
    if($idProject and Parameter::getGlobalParameter('projectDailyHours')=='true'){
    	...
    	if($project->startAM and $project->endAM and $project->startPM and $project->endPM){
    	  $startAM=($asMs)?$project->startAM:substr($project->startAM, 0, 5);
    	  $endAM=($asMs)?$project->endAM:substr($project->endAM, 0, 5);
    	  $startPM=($asMs)?$project->startPM:substr($project->startPM, 0, 5);
    	  $endPM=($asMs)?$project->endPM:substr($project->endPM, 0, 5);
    	  if($col)$time = ($asMs)?$project->$col:substr($project->$col, 0, 5);
    	}else{
...
function openHourDiffTime($startDate, $endDate, $idProject=null){
  ...
  $startD=substr($startDate,0,10);
  $startH=substr($startDate,11,5).':00';
  $endD=substr($endDate,0,10);
  $endH=substr($endDate,11,5).':00';
  ...
  $dayDelay = workDayDiffDates($startD, $endD);
