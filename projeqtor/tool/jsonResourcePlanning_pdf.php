...
$ass=new Assignment();
$res=new Resource();
$querySelect .= "pe.id idPe, pe.wbs wbs, pe.wbsSortable wbsSortable, pe.priority priority, pe.idplanningmode idplanningmode, pe.validatedenddate, pe.notplannedwork, ass.* , usr.fullName as name, pe.refName refName";
...
if (isset($debugJsonQuery) and $debugJsonQuery) { // Trace in configured to
  debugTraceLog("jsonResourcePlanning_pdf: ".$query); // Trace query
...
function exportGantt($list) {
  ...
  $currencyPosition=Parameter::getGlobalParameter('currencyPosition');
	...
	foreach ($resourceList as $resource) {
		...
		echo "<Name>" . htmlencode($resource->name) . "</Name>" . $nl;
