...
function kanbanAddPrinc($line) {
	...
	if ($kanbanFullWidthElement == "on") {
		if ($line ['idstatus']) {
			$addInPrinc .= '<div style="float:left;margin-bottom:1px;margin-left:3px;">' . colorNameFormatter ( SqlList::getNameFromId ( "Status", $line ['idstatus'] ) . '#split#' . SqlList::getFieldFromId ( "Status", $line ['idstatus'], 'color' ), $line ['id'] ) . '</div>';
		}
