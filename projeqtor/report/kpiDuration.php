...
if ($idProject) {
  ...
  if (isset($start) and isset($end)) {
    $where.=" and ( ";
    $where.="    exists (select 'x' from $khTable kh where kh.refType='Project' and kh.refId=$pjTable.id and kh.kpiDate>='$start' and kh.kpiDate<='$end' ) ";
    $where.=" or exists (select 'x' from $kvTable kv where kv.refType='Project' and kv.refId=$pjTable.id and kv.kpiDate>='$start' and kv.kpiDate<='$end' ) ";
...
foreach($listProjects as $prj) {
  ...
  if ($kpiColoFull) {
    echo '<td class="reportTableData" style="width:20%;background-color:'.$color.';text-align:left">' . (($kpiValue->kpiValue)?htmlDisplayColoredFull($kpiValue->kpiValue, $color):'') . '</td>';
  } else {
    echo '<td class="reportTableDataSpanned" style="width:20%;text-align:left">' . (($kpiValue->kpiValue)?htmlDisplayColored($kpiValue->kpiValue, $color):'') . '</td>';
  }
  $sumValues+=$kpiValue->kpiValue;
  echo '</tr>';
}
if ($cptProjectsDisplayed>0 and $scope=='Organization') {
  ...
  $consolidated=$sumValues/$cptProjectsDisplayed;
  $color='#ffffff';
  foreach ($thresholds as $th) {
...
/* Draw the scale */
$dataSet->setAxisUnit(0,"  ");
$graph->setGraphArea(40,70,$graphWidth-20,$graphHeight-60);
$graph->drawFilledRectangle(40,70,$graphWidth-20,$graphHeight-60,array("R"=>230,"G"=>230,"B"=>230));
...
if ($showThreshold) {
  ...
  foreach ($thresholds as $th) {
    ...
    $cpt++;
    $arrTh=array();
    foreach ($arrDates as $dt) {$arrTh[]=$th->thresholdValue;}
